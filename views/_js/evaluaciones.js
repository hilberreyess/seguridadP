...
jQuery(document).ready(function(){
    ...
    var tabla=$('#tablaEvaluaciones').DataTable({
        "processing": true,
        "serverSide": true,
        "responsive":true,
        "ajax": "models/ssp/ssp.evaluaciones.php",
        "language": {
        ...
        "initComplete": function (settings, json) {
            $("#tablaEvaluaciones_filter").remove();
        }
    ...
    $("#txtBuscar").keyup(function(){
        ...
        if($(this).val().length == 0){
            $("#txtBuscar").val('');
            $('#tablaEvaluaciones').DataTable().ajax.reload(null, false);
    ...
    cat_tipo_evaluaciones("00");

    $("#modal-agregar-evaluaciones").on('hidden.bs.modal', function(e) {        
        $("#form-guardar-evaluaciones").resetForm();
        $('#form-guardar-evaluaciones').bootstrapValidator('resetForm', true);

        //$ControlPDFUpload.fileinput('destroy');
        window.location.reload();
    });

    $("#modal-agregar-evaluaciones").on('shown.bs.modal', function(e) {
    ...
    $("#txtFecha").datepicker({
        ...
        onSelect: function (selectedDate) {
            //$('#form-guardar-evaluaciones').bootstrapValidator('enableFieldValidators', 'txtFecha', 'notEmpty', false);
        }
    });



    $("#form-guardar-evaluaciones").bootstrapValidator({
        excluded: [':disabled'],
        button: {
        ...
        submitHandler: function(validator, form, submitButton) {
            ...
            $.ajax({
                url: 'models/ajax/datos_evaluaciones_cmd.php',
                data: $("#form-guardar-evaluaciones").serialize()+"&xCMD="+operacion+"&xToken="+xtoken,
                type: 'POST',
                dataType: 'JSON',
                beforeSend: function() {
                        ...
                        if (respuesta.comando=="Agregar") {
                            if (respuesta.procesado) {
                                setTimeout(() => {
                                    ...
                                    $('.loading2').hide();
                                    $('#tablaEvaluaciones').DataTable().ajax.reload(null, false);
                                    $('#modal-agregar-evaluaciones').modal('hide');
                                }, 2000);
...
function editar_evaluacion(id_evaluacion){
    let xtoken    = $("#Token_CSRF").val();
    let operacion = $("#hOperacion").val();

    $(".titulo-modal").html("Editando Evaluación de Personal");
    $("#hOperacion").val("Editar");
    $("#hIdEvaluacion").val(id_evaluacion);

    $("#modal-agregar-evaluaciones").modal({
                        backdrop: 'static',
                        keyboard: true,
                        keyboard: false
                    });
    ...
    $.ajax({
        url: 'models/ajax/datos_evaluaciones_cmd.php',
        data: {"xCMD":"Seleccionar","xToken":xtoken,"xid_evaluacion":id_evaluacion},
        type: 'POST',
        dataType: 'JSON',
        beforeSend: function() {
        ...
        success: function(respuesta) {
          if (respuesta.obtenido) {
            let p = respuesta.evaluacion;

            //Asignar los datos obtenidos 
            $(".infoNoEmpleado").html(p.num_empleado);
            $(".infoNombre").html(p.nombre_completo);
            $("#txtFecha").datepicker("setDate", p.fecha_evaluacion);
            $("#selectTipoEvaluacion").selectpicker('val',p.id_tipo_evaluacion); 
            $("#selectTipoEvaluacion").selectpicker("refresh");
            $("#txtResultado").val(p.resultado);
            ...
            $ControlPDFUpload.fileinput({
                ...
                initialPreview: [
                    'views/archivos/documentos/evaluacion/'+p.documento_pdf
                ],
                ...
                uploadExtraData: function (previewId, index) {
                    var data = {
                        ...
                        xid_tipo_archivo: $("#selectTipoEvaluacion").val(),
                        xAleatorio      : $("#hAleatorio").val(),
                        xDirectorio     : "evaluacion"
                    };
                    return data;
                },
                ...
                $("#hArchivoPDF").val("");
                $("#form-guardar-evaluaciones").bootstrapValidator('enableFieldValidators', 'FileUploadPDF', true, 'notEmpty');
            ...
            $("#form-guardar-evaluaciones").bootstrapValidator('enableFieldValidators', 'FileUploadPDF', false, 'notEmpty');
...
function seleccionar_personal(id_personal, no_empleado, nombre_completo){
    ...
    aregar_evaluacion(id_personal, no_empleado, nombre_completo);
}

function aregar_evaluacion(id_personal, no_empleado, nombre_completo){
    let FechaHoy = moment(); 
    $(".titulo-modal").html("Agregar Evaluación de Personal");
...
function imprimir_documento(documento_pdf){
    if (documento_pdf.trim().length !== 0) {
        ...
        setTimeout(() => {
            ...
            $("#FramePDFjs").attr('src','views/plugins/pdfjs-2.3.2/web/viewer.php?file=../../../../views/archivos/documentos/evaluacion/'+documento_pdf+'#locale=es-MX');
...
function cat_tipo_evaluaciones(seleccionado='00'){
    $.ajax({
        url: 'models/ajax/ajax_get_catalogos.php',
        data: {"xCMD":"cat_tipo_evaluaciones"},
        type: 'POST',
        dataType: 'JSON',
        beforeSend: function() {
          $("#selectTipoEvaluacion").empty();
          $("#selectTipoEvaluacion").selectpicker("refresh");
        },
        success: function(respuesta) {
          $("#selectTipoEvaluacion").empty();
          $("#selectTipoEvaluacion").html(respuesta.evaluaciones);
          $("#selectTipoEvaluacion").selectpicker("refresh");

          if (seleccionado != '00') {
            $("#selectTipoEvaluacion").selectpicker('val',seleccionado);
            $("#selectTipoEvaluacion").selectpicker("refresh");
          }
        },
        error: function(jqXHR, status, error) {

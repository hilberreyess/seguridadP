...
jQuery(document).ready(function(){
    ...
    var tabla=$('#tablaFormaciones').DataTable({
        "processing": true,
        "serverSide": true,
        "responsive":true,
        "ajax": "models/ssp/ssp.formacion.php",
        "language": {
        ...
        "rowCallback": function(row, data, index) {
            $('td:eq(2),td:eq(3),td:eq(6),td:eq(7),td:eq(8)', row).css({"text-align":"center"});
            $('td', row).css({"vertical-align":"middle"});
            // $('td:eq(2)', row).css({"font-weight":"bold"});
        },
        "initComplete": function (settings, json) {
            $("#tablaFormaciones_filter").remove();
        }
    ...
    $("#txtBuscar").keyup(function(){
        ...
        if($(this).val().length == 0){
            $("#txtBuscar").val('');
            $('#tablaFormaciones').DataTable().ajax.reload(null, false);
    ...
    cat_tipo_formacion("00");

    $("#modal-agregar-formaciones").on('hidden.bs.modal', function(e) {        
        $("#form-guardar-formaciones").resetForm();
        $('#form-guardar-formaciones').bootstrapValidator('resetForm', true);

        //$ControlPDFUpload.fileinput('destroy');
        window.location.reload();
    });

    $("#modal-agregar-formaciones").on('shown.bs.modal', function(e) {
        
    });

    $("#txtFechaCurso").datepicker({
        inline: false,
        ...
        onSelect: function (selectedDate) {
           // $('#form-guardar-formaciones').bootstrapValidator('enableFieldValidators', 'txtFecha', 'notEmpty', false);
        }
    });



    $("#form-guardar-formaciones").bootstrapValidator({
        excluded: [':disabled'],
        button: {
        ...
        submitHandler: function(validator, form, submitButton) {
            ...
            $.ajax({
                url: 'models/ajax/datos_formacion_cmd.php',
                data: $("#form-guardar-formaciones").serialize()+"&xCMD="+operacion+"&xToken="+xtoken,
                type: 'POST',
                dataType: 'JSON',
                beforeSend: function() {
                        ...
                        if (respuesta.comando=="Agregar") {
                            if (respuesta.procesado) {
                                setTimeout(() => {
                                    ...
                                    $('.loading2').hide();
                                    $('#tablaFormaciones').DataTable().ajax.reload(null, false);
                                    $('#modal-agregar-formaciones').modal('hide');
                                }, 2000);
...
function editar_formacion(id_formacion){
    let xtoken    = $("#Token_CSRF").val();
    let operacion = $("#hOperacion").val();

    $(".titulo-modal").html("Editando Formación de Personal");
    $("#hOperacion").val("Editar");
    $("#hIdFormacion").val(id_formacion);

    $("#modal-agregar-formaciones").modal({
                        backdrop: 'static',
                        keyboard: true,
                        keyboard: false
                    });
    ...
    $.ajax({
        url: 'models/ajax/datos_formacion_cmd.php',
        data: {"xCMD":"Seleccionar","xToken":xtoken,"xid_formacion":id_formacion},
        type: 'POST',
        dataType: 'JSON',
        beforeSend: function() {
        ...
        success: function(respuesta) {
          ...
          if (respuesta.obtenido) {
            let p = respuesta.formacion;

            //Asignar los datos obtenidos 
            $(".infoNoEmpleado").html(p.num_empleado);
            $(".infoNombre").html(p.nombre_completo);
            $("#txtFechaCurso").datepicker("setDate", p.fecha_curso);
            $("#selectTipoFormacion").selectpicker('val',p.id_tipo_formacion); 
            $("#selectTipoFormacion").selectpicker("refresh");
            $("#txtNombreCurso").val(p.nombre_curso);
            ...
            $ControlPDFUpload.fileinput({
                ...
                initialPreview: [
                    'views/archivos/documentos/formacion/'+p.documento_pdf
                ],
                ...
                uploadExtraData: function (previewId, index) {
                    var data = {
                        ...
                        xid_tipo_archivo: $("#selectTipoFormacion").val(),
                        xAleatorio      : $("#hAleatorio").val(),
                        xDirectorio     : "formacion"
                    };
                    return data;
                },
                ...
                $("#hArchivoPDF").val("");
                $("#form-guardar-formaciones").bootstrapValidator('enableFieldValidators', 'FileUploadPDF', true, 'notEmpty');
            ...
            $("#form-guardar-formaciones").bootstrapValidator('enableFieldValidators', 'FileUploadPDF', false, 'notEmpty');
...
function imprimir_documento(documento_pdf){
    if (documento_pdf.trim().length !== 0) {
        ...
        setTimeout(() => {
            ...
            $("#FramePDFjs").attr('src','views/plugins/pdfjs-2.3.2/web/viewer.php?file=../../../../views/archivos/documentos/formacion/'+documento_pdf+'#locale=es-MX');
...
function cat_tipo_formacion(seleccionado='00'){
    $.ajax({
        url: 'models/ajax/ajax_get_catalogos.php',
        data: {"xCMD":"cat_tipo_formacion"},
        type: 'POST',
        dataType: 'JSON',
        beforeSend: function() {
          $("#selectTipoFormacion").empty();
          $("#selectTipoFormacion").selectpicker("refresh");
        },
        success: function(respuesta) {
          $("#selectTipoFormacion").empty();
          $("#selectTipoFormacion").html(respuesta.formacion);
          $("#selectTipoFormacion").selectpicker("refresh");

          if (seleccionado != '00') {
            $("#selectTipoFormacion").selectpicker('val',seleccionado);
            $("#selectTipoFormacion").selectpicker("refresh");
          }
        },
        error: function(jqXHR, status, error) {
